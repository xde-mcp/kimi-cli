...
async fn load_system_prompt(
    ...
    substitute_template(system_prompt.trim(), &values)
}
...
fn substitute_template(
    ...
    let re = Regex::new(r"\$\{([A-Za-z0-9_]+)\}|\$([A-Za-z0-9_]+)").map_err(|err| {
        AgentSpecError::new(format!("Failed to build system prompt template: {err}"))
    })?;
    ...
    if !missing.is_empty() {
        ...
        return Err(AgentSpecError::new(format!(
            "Missing system prompt arguments: {}",
            missing.join(", ")
        )));
    }
